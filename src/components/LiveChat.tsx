...
export default function LiveChat({ session }: { session: Session }) {
            ...
            supabase
              ...
              .then(({ data }) => {
                setMessages(prev => [...prev, {
                  ...
                  user_name: data?.display_name || data?.full_name || "Unknown"
                }]);
                scrollToBottom();
              });
          }
    ...
    try {
      ...
      if (data) {
        const formattedMessages = data.map((item: any) => ({
          ...
          user_name: item.profiles?.display_name || item.profiles?.full_name || "Unknown",
        }));
  ...
  if (loading && !activeRoom) return <div>Loading chat rooms...</div>;
      ...
      <div className="chat-area">
        ...
        <div className="chat-messages">
          {messages.map((msg) => (
            <div 
              key={msg.id} 
              className={`chat-bubble ${msg.user_id === session.user.id ? 'sent' : 'received'}`}
            >

...
CREATE POLICY "Only business owners can create hours"
ON public.business_inside_hours
FOR INSERT
TO authenticated
WITH CHECK (
    EXISTS (
...
CREATE POLICY "Only business owners can update hours"
ON public.business_inside_hours
FOR UPDATE
TO authenticated
USING (
...
CREATE POLICY "Only business owners can delete hours"
ON public.business_inside_hours
FOR DELETE
TO authenticated
USING (
...
CREATE OR REPLACE FUNCTION update_business_inside_hours_updated_at()
RETURNS TRIGGER AS $$
BEGIN
    NEW.updated_at = NOW();
    RETURN NEW;
...
-- Création du trigger
CREATE TRIGGER update_business_inside_hours_updated_at
BEFORE UPDATE ON public.business_inside_hours
FOR EACH ROW
EXECUTE FUNCTION update_business_inside_hours_updated_at(); 